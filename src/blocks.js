( function ( wp ) {
	...
	// ============================================================
	// Block: rcmi/card-grid
	// Section with heading, note, and 4-card grid.
	// ============================================================
	registerBlockType( 'rcmi/card-grid', {
		apiVersion: 3,
		title: __( 'RCMI Card Grid', 'rcmi-toolkit' ),
		description: __( 'Section with heading, note, and a 4-column card grid.', 'rcmi-toolkit' ),
		category: 'rcmi-sections',
		icon: 'grid-view',
		supports: { html: false, align: [ 'full', 'wide' ] },
		attributes: {
			heading: { type: 'string', default: 'Section heading goes <strong>here</strong>' },
			note:    { type: 'string', default: 'A short description of what this section covers and why it matters.' },
			card1Tag:   { type: 'string', default: 'Tag' },
			card1Title: { type: 'string', default: 'Card Title' },
			card1Desc:  { type: 'string', default: 'Card description goes here. Keep it concise and actionable.' },
			card2Tag:   { type: 'string', default: 'Tag' },
			card2Title: { type: 'string', default: 'Card Title' },
			card2Desc:  { type: 'string', default: 'Card description goes here. Keep it concise and actionable.' },
			card3Tag:   { type: 'string', default: 'Tag' },
			card3Title: { type: 'string', default: 'Card Title' },
			card3Desc:  { type: 'string', default: 'Card description goes here. Keep it concise and actionable.' },
			card4Tag:   { type: 'string', default: 'Tag' },
			card4Title: { type: 'string', default: 'Card Title' },
			card4Desc:  { type: 'string', default: 'Card description goes here. Keep it concise and actionable.' }
		},
		edit: function ( props ) {
			var attrs = props.attributes, setAttributes = props.setAttributes;
			var blockProps = useBlockProps( { className: 'rcmi-card-grid-editor' } );
			var cardFields = function ( n ) {
				var prefix = 'card' + n;
				return el( PanelBody, { title: __( 'Card ' + n, 'rcmi-toolkit' ), initialOpen: n === 1 },
					el( TextControl, { label: __( 'Tag', 'rcmi-toolkit' ), value: attrs[prefix + 'Tag'], onChange: function ( v ) { var u = {}; u[prefix + 'Tag'] = v; setAttributes( u ); } } ),
			...
			var cardEl = function ( n ) {
				var prefix = 'card' + n;
				return el( 'div', { className: 'card' },
					el( 'span', { className: 'tag' }, attrs[prefix + 'Tag'] ),
			...
			return el( Fragment, null,
				el( InspectorControls, null,
					el( PanelBody, { title: __( 'Section Header', 'rcmi-toolkit' ), initialOpen: true },
						el( TextareaControl, { label: __( 'Heading (HTML allowed)', 'rcmi-toolkit' ), value: attrs.heading, onChange: function ( v ) { setAttributes( { heading: v } ); } } ),
						el( TextareaControl, { label: __( 'Note', 'rcmi-toolkit' ), value: attrs.note, onChange: function ( v ) { setAttributes( { note: v } ); } } )
					),
					cardFields( 1 ), cardFields( 2 ), cardFields( 3 ), cardFields( 4 )
				),
				el( 'section', blockProps,
					el( 'div', { className: 'wrap' },
						el( 'div', { className: 'section-head' },
							el( 'div', null, el( 'h2', { dangerouslySetInnerHTML: { __html: attrs.heading } } ) ),
							el( 'p', { className: 'section-note' }, attrs.note )
						),
						el( 'div', { className: 'card-grid' },
							cardEl( 1 ), cardEl( 2 ), cardEl( 3 ), cardEl( 4 )
						)
					)
				)
			);
